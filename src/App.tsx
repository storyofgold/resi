...
function extractKecKota(t: string): string {
  const s = String(t ?? '');
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          <div className="hidden sm:flex gap-2">
            <button
              onClick={() => { setRows([]); setFiles([]); toast('Data dibersihkan.', 'info'); }}
              className="btn px-3 py-2 rounded-xl text-sm"
            >Clear</button>
          ...
          <aside className="lg:col-span-5 glass rounded-3xl p-5 sm:p-6">
            ...
            >
              ...
              <div
                ...
                style={{ color: libOk ? '#6ee7b7' : '#fca5a5' }}
              >
                {libOk ? '✓ PDF.js 4.10.38 loaded dan siap.' : '✗ PDF.js belum/gagal load.'}
              </div>
              <ul
                className="mt-3 text-sm space-y-1 list-disc pl-5"
        ...
        <main id="hasil" className="mt-6 sm:mt-8">
          <section className="glass rounded-3xl p-5 sm:p-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                />
                <button
                  onClick={() => { copyTSV(rows); toast('TSV disalin!', 'ok'); }}
                  className="btn px-3 py-2 rounded-xl text-sm"
                >Copy TSV</button>
                <button
                  onClick={() => exportCSV(rows)}
                  className="btn btn-green px-3 py-2 rounded-xl text-sm"
                >Export CSV</button>
              </div>
            </div>

            <div
              className="mt-4 overflow-auto rounded-2xl"
          ...
          <footer className="py-10 text-center text-xs" style={{ color: 'rgba(255,255,255,.40)' }}>Resi PDF Parser — Hans Logistik</footer>
        </main>
      </div>
      ...
      <div className="fixed right-4 bottom-4 z-50 flex flex-col gap-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`glass ${toastBg[t.type]} rounded-2xl px-4 py-3 text-sm`}
      ...
      <style>{`
        ...
        .btn-primary {
          background: linear-gradient(135deg, rgba(124,58,237,.95), rgba(99,102,241,.90));
        ...
        .drop.dragover {
          outline-color: rgba(124,58,237,.75);
